...
def generate_next_month_scenarios(
    ...
    return_features : bool, default False
        If True, also return the scaled RNN conditioning outputs used by the
        diffusion model. These features are the natural state features for an
        RL scenario pool, matching the linked repo's use of ``scenario['feature']``.
    ...
    If return_features is True:
        tuple[torch.Tensor, torch.Tensor]
            scenarios: shape [num_scenarios, num_assets]
            rnn_features: shape [num_scenarios, rnn_hidden_dim]
    """
    model.eval()
    device = next(model.parameters()).device
    ...
    samples, features = model.sample_autoregressive(
    ...
    samples_next = samples[0].detach().cpu()
    features_next = features[0].detach().cpu()

    if return_features:
        return samples_next, features_next
    return samples_next
